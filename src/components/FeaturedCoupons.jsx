    ...
    async function fetchStores() {
      try {
        ...
        allCoupons.forEach((coupon) => {
          ...
          if (name) {
            // البحث عن رابط الشعار بجميع الأسماء الممكنة داخل الكوبون
            const foundLogo = coupon.storeLogo || coupon.logo || coupon.image || coupon.img || null;
            ...
            } else if (!storeMap[name].logo && foundLogo) {
              storeMap[name].logo = foundLogo;
            }
      ...
      {loading ? (
        ...
        <div style={{
          ...
          {stores.map((store, index) => {
            // شعار ذكي باسم المتجر بدون الخلفية الوردي القديمة
            const avatarFallback = `https://ui-avatars.com/api/?name=${encodeURIComponent(store.name)}&background=f1f5f9&color=0f172a&size=128&bold=true`;
            const displayLogo = store.logo || avatarFallback;
